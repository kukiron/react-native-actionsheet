...
class ActionSheet extends React.Component {
  ...
  renderCancelButton () {
    ...
    return this.createButton(options[cancelButtonIndex], cancelButtonIndex)
  }

  createButton (title, index) {
    const styles = this.styles
  ...
  renderOptions () {
    ...
    return this.props.options.map((title, index) => {
      return cancelButtonIndex === index ? null : this.createButton(title, index)
    })
  }
  ...
  render () {
    ...
    return (
      <Modal
        ...
        animationType='none'
        transparent
        onRequestClose={this.handleCancleAction}
      >
        <View style={[styles.wrapper]}>
          <Text
            style={[styles.overlay]}
            onPress={this.handleCancleAction}
          />
          <Animated.View
            style={[
              styles.body,
          ...
          >
            {this.renderTitle()}
            {this.renderMessage()}
            <ScrollView scrollEnabled={this.scrollEnabled}>{this.renderOptions()}</ScrollView>
